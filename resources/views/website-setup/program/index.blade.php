        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    @if (hasPermission('program_create'))
                        <a href="{{ route('program.create') }}" class="btn btn-lg ot-btn-primary">
                        ...
                        <table class="table table-bordered class-table">
                            <thead class="thead">
                                <tr>
                                    ...
                                    <th>{{ ___('settings.focus_area') }}</th>
                                    <th>{{ ___('common.price') }}</th>
                                    <th>{{ ___('common.Serial') }}</th>
                                    <th>{{ ___('common.status') }}</th>
                                    @if (hasPermission('program_update') || hasPermission('program_delete'))
                                    ...
                                    <tr id="row_{{ $row->id }}">
                                        ...
                                        <td>
                                            <div class="user-avatar">
                                                <img src="{{ $row->image ?: asset('images/no_data.svg') }}" alt="" style="width:44px;height:32px;object-fit:cover;border-radius:5px">
                                            </div>
                                        </td>
                                        <td>{{ $row->title }}</td>
                                        <td>{{ @$row->category->name }}</td>
                                        <td>{{ @$row->focus->name ?? '—' }}</td>
                                        <td>{{ $row->price ?: '—' }}</td>
                                                ...
                                                <div class="dropdown dropdown-action">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                        @if (hasPermission('program_update'))
                                                            <li>
                                                                <a class="dropdown-item" href="{{ route('program.edit', $row->id) }}">
                                                                    <span class="icon mr-8"><i class="fa-solid fa-pen-to-square"></i></span>{{ ___('common.edit') }}
                                                                </a>
                                                            </li>
                                                        @endif
                                                        @if (hasPermission('program_delete'))
                                                            <li>
                                                                <a class="dropdown-item" href="javascript:void(0);"
                                                                    onclick="delete_row('program/delete', {{ $row->id }})">
